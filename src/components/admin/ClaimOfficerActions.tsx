...
const CONFIGS: Record<OfficerClaimAction, DialogConfig> = {
  start_review: {
    action: "start_review",
    title: "Start reviewing this claim?",
    description: "This will move the claim from New to Under Review and record the action in the claim history.",
...
export function ClaimOfficerActions({ claimId, status }: { claimId: string; status: string }) {
  ...
  const actions = normalized === "UNDER_REVIEW"
    ? [CONFIGS.request_more_info, CONFIGS.approve, CONFIGS.reject]
  ...
  return (
    <Card className="space-y-4">
      ...
      {actions.length > 0 ? <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap">{actions.map((config) => <Button key={config.action} type="button" variant={config.danger ? "danger" : config.action === "approve" ? "primary" : "secondary"} onClick={(event) => openDialog(config, event.currentTarget)}>{config.confirmLabel}</Button>)}</div> : null}
      ...
      {dialog ? <ActionDialog config={dialog} pending={pending} error={error} onClose={closeDialog} onConfirm={(note) => run(dialog.action, note)} /> : null}
    </Card>
  );
}
